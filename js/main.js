...
// function callback(e) {
//     bingimg = e.data.url;
//     $("#bgPreBoxInBing").css("background-image", "url(" + e.data.url + ")")
//     loadbg();
//     // 移除用于跨域的script
...
function soso(data) {
    ...
    var con = "";
    var count = 0;
    if ($(".associate_ul li").length == 0) {
        key.forEach(function(el) {
            // con += '<li onclick="goback()"><a href ="https://www.baidu.com/s?wd=' + el + '" target="_blank">' + el + '</li>';
            con += '<li onclick="jump(this)" name="' + el + '">' + el + '</li>';
            $(".associate_ul").html(con);
        });
    } else {
        key.forEach(function(el) {
            $(".associate_ul li")[count].innerText = el;
            count++;
        });
    }
...
function codeLoad() {
    login.username && typeof loggedIn == 'function' && loggedIn();
    ...
    var rthCode = document.querySelector('script[src^=\'https://api.rthsoftware.cn/backend/code\']');
    rthCode && rthCode.parentElement.removeChild(rthCode);
}

function logOut() {
    fetch('https://api.rthsoftware.cn/backend/logout?token=' + login.token, {
        credentials: 'include'
        ...
        if (response.status == 204) {
            ...
            typeof loggedOut == 'function' && loggedOut();
            location.reload();
        }
    });
}
...
window.onload = () => {
    ...
    $('#input0').bind('input propertychange', function() {
        Input_Change();
    });
}
...
function Input_Change() {
    ...
    // console.log(input0.value+'1')
    if (!input0.value || input0.value.trim().length == 0) {
        $(".associate_ul li").remove();
        // ShowHistory();
        return;
    } else {
        var oScript = document.createElement('script');
        oScript.src = 'https://sp0.baidu.com/5a1Fazu8AA54nxGko9WTAnF6hhy/su?wd=' + input0.value + '&cb=soso';
        $("body").append(oScript);
        oScript.remove();
    }
}
...
function AddHistory(str) {
    console.log('add' + str)
    var associate_ul = localStorage.getItem("associate_ul");
    var newarray = [];
    if (!associate_ul || associate_ul == "") {
        associate_ul = str;
    } else {
        associate_ul = str + "," + associate_ul;
    }
    var stringResult = associate_ul.split(',');
    for (var i = 0; i < 10 && i < stringResult.length; i++) {
        newarray[i] = stringResult[i];
    }
    var arrayResult = newarray.join(',')
    localStorage.setItem("associate_ul", arrayResult);
}
...
function ShowHistory() {
    ...
    if (!associate_ul || associate_ul == "") {
        // console.log('搜索历史为空')
        $(".associate_ul").html('<li><a href="#">暂无搜索历史</a></li>');
    } else {
        var stringResult = associate_ul.split(',');
        var con = "";
        stringResult.forEach(function(el) {
...
function Input_Focus() {
    keyConnect.style.opacity = "1"
    keyConnect.style.display = "block"


    quotebox.style.opacity = "1";
    searchOptBox.style.display = "block";
    input0.classList.add("focus");
    title.style.top = innerWidth > 600 ? "50px" : "0px";
    setTimeout(() => {
            searchOptBox.style.opacity = "1";
            searchOptBox.style.top = innerWidth > 600 ? "150px" : "65px";
        },
        100);
...
function Input_Blur() {
    ...
    keyConnect.style.opacity = "0"
    keyConnect.style.display = "none"
    input0.value = "";
...
function Navbox_Click(event) {
    ...
    if (menuDelete.style.display == 'block') {
        hideMenu(menuDelete);
        return;
    }
    if (frmSetCustomNav.style.display == 'block') {
        closeFrmCusNav()
        return;
    }
...
function changeWp(obj) {
    switch (obj.id) {
        ...
        case 'bgPreBoxBing':
            liveBgBox.style.display = "none";
            liveBgBox.pause();
            // bgbox.src = 'https://api.dujin.org/bing/1920.php';
            bgbox.src = bingimg;
            // bgbox.src = "https://www4.bing.com/az/hprichbg/rb/TheNationaDay_ZH-CN5381900085_1920x1080.jpg";
            bgbox.style.opacity = "1";
            bgPreBoxInCus.classList.remove("selected");
            bgPreBoxIn1.classList.remove("selected");
            bgPreBoxIn2.classList.remove("selected");
...
function cusNavClick(event, obj) {
    ...
    if (window.screen.availWidth > 600) {
        frmSetCustomNav.style.left = event.clientX - 150 + "px";
        frmSetCustomNav.style.top = event.clientY + 50 + "px";
    } else {
        //获得窗口的垂直位置 
        frmSetCustomNav.style.top = 200 + "px";
        //获得窗口的水平位置 
        frmSetCustomNav.style.left = (window.screen.availWidth - 10 - 300) / 2 + "px";
    }
...
function addCusNav() {
    ...
    if (isLoggedIn()) {
        if (inputCustomUrl.value) {
            ...
            currentEditingNav.innerHTML = ` < img class = "cusNavIcon shouldNotFade"src = "` + cusNavIconUrl + `"onerror = "addCusNav2(this)"onload = "addCusNav3(this);submitCusNav();" / ><div class = "cusNavTitle shouldNotFade" > ` + inputCustomTitle.value + ` < /div>`;
...
function addCusNav2(obj) {
    ...
    if (cusNavIconErrCount == 1) {
        cusNavIconUrl = "https:/ / statics.dnspod.cn / proxy_favicon / _ / favicon ? domain = " + cusNavIconUrlParsed.host;
...
function addCusNav3(obj) {
    if (obj.naturalWidth < 17) {
        ...
        if (cusUrlInitial.startsWith("www.")) { cusUrlInitial = cusUrlInitial.substring(4); }
...
function showDeleteMenu(e) {
    menuDelete.style.left = e.clientX + 3 + "px";
    menuDelete.style.top = e.clientY + 3 + "px";
    showMenu(menuDelete);
}
